...
void MiriGenmcShim::handle_old_val(const SAddr addr, GenmcScalar value) {
    LOG(VerbosityLevel::Warning) << "handle_old_val: " << addr << ", " << value.value << ", "
    ...
    if (auto* wLab = llvm::dyn_cast<WriteLabel>(coLab)) {
        LOG(VerbosityLevel::Warning)
            << "handle_old_val: got WriteLabel, atomic: " << !wLab->isNotAtomic() << "\n";
        if (!value.is_init)
            LOG(VerbosityLevel::Warning) << "WARNING: TODO GENMC: handle_old_val tried to "
        ...
        if (value.is_init) {
            ...
            LOG(VerbosityLevel::Warning) << "WARNING: TODO GENMC: handle_old_val tried set initial "
                                            "value, but old "
                                            "value is `uninit`\n";
        }
    } else {
